...
def compress_gamepad(gamepadInput):
    ...
    struct.pack_into('h', arr, 14, max(min(int(float(gamepadAnalogs[6]) * 2 - 1), 1), -1))
    struct.pack_into('h', arr, 16, max(min(int(float(gamepadAnalogs[7]) * 2 - 1), 1), -1))
    #eprint(gamepadInput)
    return arr

def decompress_gamepad_binary(binary):
    arr = np.zeros(numInputs)
    ...
    arr[4] = 1.0 if (binary[0] & 0x10) else 0.0
    arr[5] = 1.0 if (binary[0] & 0x20) else 0.0
    arr[6] = 1.0 if (binary[0] & 0x40) else 0.0
